...
Uses Gemini to create question + ground_truth pairs from random MySQL chunks,
...
def generate_question_from_chunk(client, model: str, chunk_text: str) -> dict:
    """Ask Gemini to generate a question+answer pair from a chunk."""
...
def main():
    ...
    for chunk in chunks:
        ...
        try:
            result = generate_question_from_chunk(gemini_client, judge_model, chunk['chunk_text'])
            questions.append({
                "id": len(questions) + 1,
